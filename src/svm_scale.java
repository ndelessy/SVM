...
class svm_scale
{
	...
	private BufferedWriter saveBW = null;
	...
	private static void exit_with_help()
	{
		...
		+"-o scaled_data_filename : save scaled parameters to scaled_data_filename\n"
		);
		System.exit(1);
	}
	...
	private void output_target(double value)
	{
		...
		if(saveBW != null){
			...
			try {
				saveBW.write(formatter.toString());
			} catch (IOException e) {
				e.printStackTrace();
			}
		}
	...
	private void output(int index, double value)
	{
		...
		if(value != 0)
		{
			if(saveBW != null){
				Formatter formatter = new Formatter(new StringBuilder());
				formatter.format("%d:%.16g ", index, value);
				try {
					saveBW.write(formatter.toString());
				} catch (IOException e) {
					e.printStackTrace();
	...
	private void run(String []argv) throws IOException
	{
		...
		String scaled_data_filename = null;
		...
		for(i=0;i<argv.length;i++)
		{
			...
			switch(argv[i-1].charAt(1))
			{
				...
				case 'o': 
					scaled_data_filename = argv[i];	
					if(scaled_data_filename != null) {
						try {
							saveBW = new BufferedWriter(new FileWriter(scaled_data_filename));
						} catch(IOException e) {
							System.err.println("can't open file " + scaled_data_filename);
							System.exit(1);
						}
					}
